...
def main():
    ...
    # starting thread for each devic
    for a_device in devices:
      
        my_thread = threading.Thread(target=print_output, args=(a_device,))
        my_thread.start()
    #requesting threads to wait for the main() thread
    main_thread = threading.currentThread()
    for some_thread in threading.enumerate():
        if some_thread != main_thread:
            some_thread.join()
    #calculate elapsed time when all threads terminate
